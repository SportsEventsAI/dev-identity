...
export type ItemEditData = Pick<Item, "name" | "description"> & {
  tags: ItemTagEditData[];
};

export const ItemEditForm: FC<ItemFormProps> = ({
  item,
  handleSave,
  handleClose,
}) => {
  const {
    ...
    control,
    formState: { errors },
  } = useForm<ItemEditData>({ defaultValues: item || {} });
        ...
        <div className="col-sm-10">
          ...
          {fields.map((item, index) => (
            <div className="form-row mb-1" key={item.id}>
              <div className="col-7">
                <Controller
                  name={`tags[${index}].id` as `tags.${number}.id`}
                  control={control}
                ...
                <input
                  className={`form-control ${
                    errors.tags && errors.tags[index] ? "is-invalid" : ""
                  }`}
                  {...register(`tags[${index}].name` as `tags.${number}.name`, {
                    required: true,
                ...
                {errors.tags &&
                  errors.tags[index]?.name?.type === "validate" && (
                    <div className="invalid-feedback">Invalid</div>
                  )}
              </div>
              <div className="col-5">
